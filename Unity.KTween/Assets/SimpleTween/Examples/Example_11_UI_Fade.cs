...
public class Example_11_UI_Fade : MonoBehaviour
{
    ...
    {
        // 创建完整的淡入 → 保持 → 淡出序列
        var fadeIn = SimpleTween.AddTween(gameObject, fadeInDuration, (t) =>
        {
            m_CanvasGroup.alpha = t;
        });

        var hold = SimpleTween.delayedCall(gameObject, holdDuration);

        var fadeOut = SimpleTween.AddTween(gameObject, fadeOutDuration, (t) =>
        {
