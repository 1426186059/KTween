...
/// <summary>
/// PlayMode 测试：验证 SimpleTween 在完整 MonoBehaviour 生命周期下的行为。
/// 这些测试依赖 Time.deltaTime 和 Update 循环，需要在 PlayMode 下运行。
/// </summary>
public class SimpleTweenPlayModeTests
{
    ...
    public void Teardown()
    {
        ...
        // 清理 SimpleTweenMgr 单例，防止场景卸载时残留
    ...
    {
        int callCount = 0;
        SimpleTween.AddTween(m_TestObject, 0.5f, (t) => callCount++);
    ...
    public IEnumerator TimePercent_StartsAtZero_EndsAtOne()
    {
        ...
        SimpleTween.AddTween(m_TestObject, 0.3f, (t) =>
        {
    ...
    {
        bool finished = false;
        SimpleTween.AddTween(m_TestObject, 0.2f, null, () => finished = true);
    ...
    {
        bool updated = false;
        SimpleTween.AddTween(m_TestObject, 0.2f, (t) => updated = true).SetDelay(0.5f);
    ...
    {
        bool finished = false;
        SimpleTween.AddTween(m_TestObject, 0.1f, null, () => finished = true).SetDelay(0.4f);
    ...
    public IEnumerator Tween_StopsWhenObjectDestroyed()
    {
        ...
        SimpleTween.AddTween(tempGo, 5.0f, (t) => callCount++);
    ...
    {
        int callCount = 0;
        var handle = SimpleTween.AddTween(m_TestObject, 3.0f, (t) => callCount++).GetHandle();
    ...
    {
        int callCount = 0;
        SimpleTween.AddTween(m_TestObject, 0.1f, (t) => callCount++).SetLoop(-1);
    ...
    {
        int callCount = 0;
        SimpleTween.AddTween(m_TestObject, 0.1f, (t) => callCount++).SetLoop(3);
    ...
    {
        float lastT = -1f;
        SimpleTween.AddTween(m_TestObject, 0.2f, (t) => lastT = t).SetLoopPingPong(2);
    ...
    public IEnumerator AppendTween_ExecutesInSequence()
    {
        ...
        var first = SimpleTween.AddTween(m_TestObject, 0.2f, (t) => { },
            () => { firstExec = ++executionOrder; });

        var second = SimpleTween.AddTween(m_TestObject, 0.2f, (t) => { },
    ...
    public IEnumerator Handle_AppendTween_Sequences()
    {
        ...
        var h1 = SimpleTween.AddTween(m_TestObject, 0.2f, null,
            () => first = ++order).GetHandle();
        var h2 = SimpleTween.AddTween(m_TestObject, 0.2f, null,
    ...
    public IEnumerator MultipleTweens_RunIndependently()
    {
        ...
        SimpleTween.AddTween(go1, 0.3f, (t) => pos1 = t, () => pos1 = 1f);
        SimpleTween.AddTween(go2, 0.6f, (t) => pos2 = t, () => pos2 = 1f);
    ...
    {
        bool started = false;
        SimpleTween.AddTween(m_TestObject, 0.1f, (t) => { if (t > 0) started = true; });
    ...
    public IEnumerator ManyTweens_DoesNotCrash()
    {
        ...
        for (int i = 0; i < 500; i++)
        {
            ...
            SimpleTween.AddTween(gos[i], UnityEngine.Random.Range(0.1f, 0.5f), (t) => { });
        }
    ...
    {
        int calls = 0;
        var a = SimpleTween.AddTween(m_TestObject, 5.0f, (t) => calls++);
        var b = SimpleTween.AddTween(m_TestObject, 5.0f, (t) => calls++);
        var c = SimpleTween.AddTween(m_TestObject, 5.0f, (t) => calls++);
        a.AppendTween(b);
        b.AppendTween(c);
