...
/// <summary>
/// EditMode 单元测试：测试 SimpleTween 核心 API，不依赖 MonoBehaviour 生命周期。
/// 注意：AddTween 在 EditMode 下只能验证 API 调用逻辑，不会真正驱动 Update 循环。
/// </summary>
public class SimpleTweenEditModeTests
{
    [SetUp]
    public void Setup()
    {
        // 清理单例管理器（仅对已创建的实例有效）
        if (SimpleTween.SimpleTweenMgr.Instance != null)
        {
            var go = SimpleTween.SimpleTweenMgr.Instance.gameObject;
            GameObject.DestroyImmediate(go);
        }
        // 重置最大数量
        SimpleTween.SetMaxTweenCount(1024);
    }
    ...
    {
        var item = SimpleTween.AddTween(1.0f);
    ...
    public void AddTween_WithGameObject_BindsObject()
    {
        ...
        var item = SimpleTween.AddTween(go, 1.0f);
    ...
    public void AddTween_UpdateFunc_IsInvokedDuringTime()
    {
        ...
        var item = SimpleTween.AddTween(go, 1.0f, (t) => lastT = t);
        Assert.AreEqual(0f, lastT, 0.001f); // 初始时立即被调用一次（time=0）

        // 模拟更新
        // 注意：无法直接触发 SimpleTweenByLinkedList 的 Update，
    ...
    public void AddTween_FinishFunc_IsStored()
    {
        bool finished = false;
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f, null, () => finished = true);
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetDelay(2.5f);
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetLoop(-1);
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetLoop(5);
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetLoop();
        Assert.AreEqual(-1, item.nLoopCount);
        GameObject.DestroyImmediate(go);
    }
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetLoopPingPong(-1);
        Assert.AreEqual(-1, item.nLoopCount);
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f).SetLoopPingPong(3);
    ...
    {
        var go = new GameObject("Test");
        var first = SimpleTween.AddTween(go, 1.0f);
        var second = SimpleTween.AddTween(go, 2.0f);
    ...
    {
        var go = new GameObject("Test");
        var first = SimpleTween.AddTween(go, 1.0f).SetDelay(0.5f);
    ...
    {
        var go = new GameObject("Test");
        var a = SimpleTween.AddTween(go, 1.0f);
        var b = SimpleTween.AddTween(go, 1.0f);
        var c = SimpleTween.AddTween(go, 1.0f);
        a.AppendTween(b);
        b.AppendTween(c);
    ...
    public void Static_GetHandle_ReturnsValid()
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f);
        var handle = SimpleTween.GetHandle(item);
        Assert.IsTrue(handle.IsValid());
        GameObject.DestroyImmediate(go);
    }
    ...
    public void DelayedCall_CreatesTweenWithNoUpdate()
    {
        var go = new GameObject("Test");
        bool called = false;
        var item = SimpleTween.delayedCall(go, 1.0f, () => called = true);
        Assert.IsNotNull(item);
    ...
    {
        bool called = false;
        var item = SimpleTween.delayedCall(2.0f, () => called = true);
        Assert.IsNotNull(item);
        Assert.IsNull(item.updateFunc);
        Assert.IsNotNull(item.finishFunc);
    }
    ...
    {
        Assert.DoesNotThrow(() => SimpleTween.SetMaxTweenCount(2048));
        Assert.DoesNotThrow(() => SimpleTween.SetMaxTweenCount(1));
    }
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 3.0f);
    ...
    {
        var item = SimpleTween.AddTween(1.0f);
        Assert.AreEqual(SimpleTween.SimpleTweenMgr.Instance.gameObject, item.bindObj);
    }
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 1.0f, (t) => { }, () => { });
    ...
    {
        var go = new GameObject("Test");
        var h1 = SimpleTween.AddTween(go, 1.0f).GetHandle();
        var h2 = SimpleTween.AddTween(go, 2.0f).GetHandle();
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, 0f);
        Assert.IsNotNull(item);
        Assert.AreEqual(0f, item.sumTime);
        GameObject.DestroyImmediate(go);
    }
    ...
    {
        var go = new GameObject("Test");
        var item = SimpleTween.AddTween(go, -1f);
        Assert.IsNotNull(item);
        GameObject.DestroyImmediate(go);
    }
    ...
    {
        var item = SimpleTween.AddTween(1.0f, null);
        Assert.IsNotNull(item);
    }
    ...
    {
        var go = new GameObject("Test");
        var items = new HashSet<SimpleTween.TweenItem>();
        for (int i = 0; i < 100; i++)
        {
            var item = SimpleTween.AddTween(go, 1.0f);
            items.Add(item);
    ...
    {
        var go = new GameObject("Test");
        var items = new List<SimpleTween.TweenItem>();
        for (int i = 0; i < 100; i++)
        {
            items.Add(SimpleTween.AddTween(go, 1.0f));
        }
        Assert.DoesNotThrow(() =>
        {
    ...
    public void EaseLinear_Vector3_ReturnsCorrectValue()
    {
        ...
        Assert.AreEqual(Vector3.zero, SimpleTweenFunc.easeLinear(from, to, 0f));
        Assert.AreEqual(Vector3.one * 5f, SimpleTweenFunc.easeLinear(from, to, 0.5f));
        Assert.AreEqual(Vector3.one * 10f, SimpleTweenFunc.easeLinear(from, to, 1f));
    }
    ...
    public void EaseLinear_Vector2_ReturnsCorrectValue()
    {
        ...
        Assert.AreEqual(Vector2.zero, SimpleTweenFunc.easeLinear(from, to, 0f));
        Assert.AreEqual(new Vector2(50f, 100f), SimpleTweenFunc.easeLinear(from, to, 0.5f));
        Assert.AreEqual(to, SimpleTweenFunc.easeLinear(from, to, 1f));
    }

    [Test]
    public void EaseLinear_Float_ReturnsCorrectValue()
    {
        Assert.AreEqual(0f, SimpleTweenFunc.easeLinear(0f, 100f, 0f));
        Assert.AreEqual(50f, SimpleTweenFunc.easeLinear(0f, 100f, 0.5f));
        Assert.AreEqual(100f, SimpleTweenFunc.easeLinear(0f, 100f, 1f));
    }
    ...
    public void EaseLinear_ClampsOutOfRange()
    {
        ...
        Assert.DoesNotThrow(() => SimpleTweenFunc.easeLinear(Vector3.zero, Vector3.one, -0.5f));
        Assert.DoesNotThrow(() => SimpleTweenFunc.easeLinear(Vector3.zero, Vector3.one, 1.5f));
        Assert.DoesNotThrow(() => SimpleTweenFunc.easeLinear(0f, 1f, -0.5f));
        Assert.DoesNotThrow(() => SimpleTweenFunc.easeLinear(0f, 1f, 1.5f));
    }
}
