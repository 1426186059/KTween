...
public class Example_07_CancelAndHandle : MonoBehaviour
{
    ...
    private SimpleTween.TweenItemHandle m_Handle;
    ...
    public void Play()
    {
        ...
        var item = SimpleTween.AddTween(gameObject, duration, (t) =>
        {
            if (!m_IsCancelled)
            {
                transform.position = SimpleTweenFunc.easeLinear(from, from + Vector3.right * 5f, t);
            }
        });
