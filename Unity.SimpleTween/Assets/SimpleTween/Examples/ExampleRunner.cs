...
/// <summary>
/// 示例场景主控制器：运行时生成可视化演示场景，展示所有 SimpleTween 用法
/// </summary>
public class ExampleRunner : MonoBehaviour
{
    ...
    private void CreateDemo_Combined(GameObject parent, Vector3 origin)
    {
        ...
        SimpleTween.AddTween(cube, 1.5f, (t) =>
        {
            cube.transform.position = SimpleTweenFunc.easeLinear(startPos, endPos, t);
            cube.transform.localScale = SimpleTweenFunc.easeLinear(startScale, endScale, t);
            cube.transform.eulerAngles = SimpleTweenFunc.easeLinear(Vector3.zero, new Vector3(0f, 0f, 360f), t);
        }).SetLoopPingPong(-1);
    }
