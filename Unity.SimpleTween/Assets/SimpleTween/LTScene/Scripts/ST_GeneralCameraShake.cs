...
public class ST_GeneralCameraShake : MonoBehaviour {
	...
	void bigGuyJump(){
		...
		SimpleTweenEx.moveY(avatarBig, height, 1f).setEase(SimpleTweenType.easeInOutQuad).setOnComplete( ()=>{
			SimpleTweenEx.moveY(avatarBig, 0f, 0.27f).setEase(SimpleTweenType.easeInQuad).setOnComplete( ()=>{
				SimpleTweenEx.cancel(gameObject);
				...
				**************/
				...
				LTDescr shakeTween = SimpleTweenEx.rotateAroundLocal( gameObject, Vector3.right, shakeAmt, shakePeriodTime)
				.setEase( SimpleTweenType.easeShake ) // this is a special ease that is good for shaking
				.setLoopClamp()
				.setRepeat(-1);

				// Slow the camera shake down to zero
				SimpleTweenEx.value(gameObject, shakeAmt, 0f, dropOffTime).setOnUpdate( 
					(float val)=>{
						shakeTween.setTo(Vector3.right*val);
					}
				).setEase(SimpleTweenType.easeOutQuad);
				...
				********************/
		        ...
		        SimpleTweenEx.delayedCall(2f, bigGuyJump);
			});
		});
		jumpIter += 5.2f;
	}
