...
public class ST_GeneralSequencer : MonoBehaviour {
	...
	public void Start(){
		...
		// Jump up
		var seq = SimpleTweenEx.sequence();


		seq.append( SimpleTweenEx.moveY( avatar1, avatar1.transform.localPosition.y + 6f, 1f).setEaseOutQuad() );
        ...
        seq.insert( SimpleTweenEx.alpha(star, 0f, 1f) );
        seq.insert( SimpleTweenEx.scale( star, Vector3.one * 3f, 1f) );

		// Rotate 360
		seq.append( SimpleTweenEx.rotateAround( avatar1, Vector3.forward, 360f, 0.6f ).setEaseInBack() );

		// Return to ground
		seq.append( SimpleTweenEx.moveY( avatar1, avatar1.transform.localPosition.y, 1f).setEaseInQuad() );
		...
		seq.append(() => {
			for(int i = 0; i < 50f; i++){
				...
				SimpleTweenEx.moveLocal(cloud, range, 3f*speedScale).setEaseOutCirc();
				SimpleTweenEx.rotateAround(cloud, Vector3.forward, 360f*2, 3f*speedScale).setEaseOutCirc();
				SimpleTweenEx.alpha(cloud, 0f, 3f*speedScale).setEaseOutCirc().setDestroyOnComplete(true);
			}
		});
        ...
        //SimpleTweenEx.delayedCall(3f, () =>
        //{
        //    SimpleTweenEx.cancel(seq.id);
        //});
	}
}
