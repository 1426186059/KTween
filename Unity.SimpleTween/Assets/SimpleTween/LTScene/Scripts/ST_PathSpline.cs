...
public class ST_ExampleSpline : MonoBehaviour {
	...
	void Start () {
		...
		SimpleTweenEx.moveSpline( ltLogo2, spline.pts, 1f).setEase(SimpleTweenType.easeInOutQuad).setLoopPingPong().setOrientToPath(true);

		LTDescr zoomInPath_LT = SimpleTweenEx.moveSpline(ltLogo2, new Vector3[]{Vector3.zero, Vector3.zero, new Vector3(1,1,1), new Vector3(2,1,1), new Vector3(2,1,1)}, 1.5f);
