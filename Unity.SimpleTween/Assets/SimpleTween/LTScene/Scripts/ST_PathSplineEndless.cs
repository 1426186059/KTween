...
public class ST_PathSplineEndless : MonoBehaviour {
	...
	void Start () {
		...
		SimpleTweenEx.value(gameObject, 0, 0.3f, 2f).setOnUpdate( ( float val )=>{
			pushTrackAhead = val;
		});
	}
	...
	void Update () {
		...
		if(Input.anyKeyDown){
			...
			SimpleTweenEx.moveLocalX(carInternal, (trackIter-1)*6f, 0.3f).setEase(SimpleTweenType.easeOutBack);
		}
	}
	...
	void addRandomTrackPoint(){
		...
		SimpleTweenEx.rotateAround( tree, Vector3.forward, 0f, 1f).setFrom( zIter%2==0 ? 180f : -180f).setEase(SimpleTweenType.easeOutBack);
