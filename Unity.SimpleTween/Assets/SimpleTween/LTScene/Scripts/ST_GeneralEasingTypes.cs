...
public class ST_GeneralEasingTypes : MonoBehaviour {
	...
	private void demoEaseTypes(){
		for(int i = 0; i < easeTypes.Length; i++){
			...
			float obj1val = 0f;
			LTDescr lt = SimpleTweenEx.value( obj1.gameObject, 0f, 1f, 5f).setOnUpdate( (float val)=>{
		...
		SimpleTweenEx.delayedCall(gameObject, 10f, resetLines);
		SimpleTweenEx.delayedCall(gameObject, 10.1f, demoEaseTypes);
	}
