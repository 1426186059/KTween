...
public class ST_GeneralEventsListeners : MonoBehaviour {
	...
	void Awake(){
		SimpleTweenEx.LISTENERS_MAX = 100; // This is the maximum of event listeners you will have added as listeners
		SimpleTweenEx.EVENTS_MAX = (int)MyEvents.LENGTH; // The maximum amount of events you will be dispatching
	...
	void Start () {
		...
		SimpleTweenEx.addListener(gameObject, (int)MyEvents.CHANGE_COLOR, changeColor);
		SimpleTweenEx.addListener(gameObject, (int)MyEvents.JUMP, jumpUp);
	}
	...
	void changeColor( LTEvent e ){
		...
		SimpleTweenEx.value( gameObject, fromColor, to, 0.8f ).setLoopPingPong(1).setDelay(distance*0.05f).setOnUpdate(
			(Color col)=>{
	...
	void OnMouseDown(){
		if(Input.GetKey( KeyCode.J )){ // Are you also pressing the "j" key while clicking
			SimpleTweenEx.dispatchEvent((int)MyEvents.JUMP);
		}else{
			SimpleTweenEx.dispatchEvent((int)MyEvents.CHANGE_COLOR, transform); // with every dispatched event, you can include an object (retrieve this object with the *.data var in LTEvent)
		}
	}
}
#endif
