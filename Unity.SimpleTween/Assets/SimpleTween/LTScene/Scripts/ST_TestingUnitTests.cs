...
namespace DentedPixel.LTExamples
{
    ...
    public class ST_TestingUnitTests : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            SimpleTweenEx.init(1300);

            // add a listener
            SimpleTweenEx.addListener(cube1, 0, eventGameObjectCalled);

            LeanTest.expect(SimpleTweenEx.isTweening() == false, "NOTHING TWEEENING AT BEGINNING");

            LeanTest.expect(SimpleTweenEx.isTweening(cube1) == false, "OBJECT NOT TWEEENING AT BEGINNING");

            SimpleTweenEx.scaleX(cube4, 2f, 0f).setOnComplete(() => {
                LeanTest.expect(cube4.transform.localScale.x == 2f, "TWEENED WITH ZERO TIME");
            });

            // dispatch event that is received
            SimpleTweenEx.dispatchEvent(0);
            LeanTest.expect(eventGameObjectWasCalled, "EVENT GAMEOBJECT RECEIVED");

            // do not remove listener
            LeanTest.expect(SimpleTweenEx.removeListener(cube2, 0, eventGameObjectCalled) == false, "EVENT GAMEOBJECT NOT REMOVED");
            // remove listener
            LeanTest.expect(SimpleTweenEx.removeListener(cube1, 0, eventGameObjectCalled), "EVENT GAMEOBJECT REMOVED");

            // add a listener
            SimpleTweenEx.addListener(1, eventGeneralCalled);

            // dispatch event that is received
            SimpleTweenEx.dispatchEvent(1);
            LeanTest.expect(eventGeneralWasCalled, "EVENT ALL RECEIVED");

            // remove listener
            LeanTest.expect(SimpleTweenEx.removeListener(1, eventGeneralCalled), "EVENT ALL REMOVED");

            lt1Id = SimpleTweenEx.move(cube1, new Vector3(3f, 2f, 0.5f), 1.1f).id;
            SimpleTweenEx.move(cube2, new Vector3(-3f, -2f, -0.5f), 1.1f);
            ...
            for (int i = 0; i < cubes.Length; i++)
            {
                ...
                tweenIds[i] = SimpleTweenEx.moveX(c, 100f, 1f).id;
                cubes[i] = c;
            }
            int onCompleteCount = 0;
            SimpleTweenEx.delayedCall(cubes[0], 0.2f, () => {
                for (int i = 0; i < cubes.Length; i++)
                {
                    if (i % 3 == 0)
                    {
                        SimpleTweenEx.cancel(cubes[i]);
                    }
                    else if (i % 3 == 1)
                    {
                        SimpleTweenEx.cancel(tweenIds[i]);
                    }
                    else if (i % 3 == 2)
                    {
                        LTDescr descr = SimpleTweenEx.descr(tweenIds[i]);
            ...
            SimpleTweenEx.color(cube4, Color.green, 0.01f);
            ...
            SimpleTweenEx.move(cubeDest, cubeDestEnd, 0.7f);

            GameObject cubeToTrans = cubeNamed("cubeToTrans");
            SimpleTweenEx.move(cubeToTrans, cubeDest.transform, 1.2f).setEase(SimpleTweenType.easeOutQuad).setOnComplete(() => {
            ...
            SimpleTweenEx.moveX(cubeDestroy, 200f, 0.05f).setDelay(0.02f).setDestroyOnComplete(true);
            SimpleTweenEx.moveX(cubeDestroy, 200f, 0.1f).setDestroyOnComplete(true).setOnComplete(() => {
                LeanTest.expect(true, "TWO DESTROY ON COMPLETE'S SUCCEED");
            });

            GameObject cubeSpline = cubeNamed("cubeSpline");
            SimpleTweenEx.moveSpline(cubeSpline, new Vector3[] { new Vector3(0.5f, 0f, 0.5f), new Vector3(0.75f, 0f, 0.75f), new Vector3(1f, 0f, 1f), new Vector3(1f, 0f, 1f) }, 0.1f).setOnComplete(() => {
            ...
            int jumpTimeId = SimpleTweenEx.moveX(jumpCube, 200f, 1f).id;

            SimpleTweenEx.delayedCall(gameObject, 0.2f, () => {
                LTDescr d = SimpleTweenEx.descr(jumpTimeId);
                float beforeX = jumpCube.transform.position.x;
                d.setTime(0.5f);
                SimpleTweenEx.delayedCall(0.0f, () => { }).setOnStart(() => {
            ...
            SimpleTweenEx.moveX(zeroCube, 10f, 0f).setOnComplete(() => {
            ...
            SimpleTweenEx.scale(cubeScale, new Vector3(5f, 5f, 5f), 0.01f).setOnStart(() => {
            ...
            SimpleTweenEx.rotate(cubeRotate, new Vector3(0f, 180f, 0f), 0.02f).setOnComplete(() => {
            ...
            SimpleTweenEx.rotateAround(cubeRotateA, Vector3.forward, 90f, 0.3f).setOnComplete(() => {
            ...
            SimpleTweenEx.rotateAround(cubeRotateB, Vector3.forward, 360f, 0.3f).setPoint(new Vector3(5f, 3f, 2f)).setOnComplete(() => {
            ...
            SimpleTweenEx.alpha(cubeAlpha1, 0.5f, 0.1f).setOnUpdate((float val) => {
            ...
            SimpleTweenEx.color(cubeAlpha2, Color.cyan, 0.3f).setOnComplete(() => {
            ...
            SimpleTweenEx.moveY(cubeAlpha1, 3f, 0.2f).setOnComplete(() => {
            ...
            SimpleTweenEx.moveLocalZ(cubeAlpha2, 12f, 0.2f).setOnComplete(() => {
            ...
            SimpleTweenEx.delayedSound(gameObject, audioClip, new Vector3(0f, 0f, 0f), 0.1f).setDelay(0.2f).setOnComplete(() => {
            ...
            for (int j = 0; j < 2; j++)
            {
                ...
                int totalTweenTypeLength = (int)SimpleTweenType.easeShake;
                for (int i = (int)SimpleTweenType.notUsed; i < totalTweenTypeLength; i++)
                {
                    SimpleTweenType easeType = (SimpleTweenType)i;
                    GameObject cube = cubeNamed("cube" + easeType);
                    LTDescr descr = SimpleTweenEx.moveLocalX(cube, 5, 0.1f).setOnComplete((object obj) => {
            ...
            SimpleTweenEx.value(gameObject, new Vector2(0, 0), new Vector2(256, 96), 0.1f).setOnUpdate((Vector2 value) => {
                value2UpdateCalled = true;
            });
            SimpleTweenEx.delayedCall(0.2f, () => {
            ...
            SimpleTweenEx.cancel(cubeCancelA, false, TweenAction.MOVE_X);
            LTDescr[] descrs = SimpleTweenEx.descriptions(cubeCancelA);
            LeanTest.expect(descrs.Length == 1, "SPECIFIC TWEEN DID CANCEL");


            // check descr
            //          LTDescr descr2 = SimpleTweenEx.descr( descrId );
        ...
        IEnumerator timeBasedTesting()
        {
            ...
            SimpleTweenEx.moveX(cubeNormal, 12f, 1.5f).setIgnoreTimeScale(false).setOnComplete(() => {
                timeElapsedNormalTimeScale = Time.time;
            });

            LTDescr[] descr = SimpleTweenEx.descriptions(cubeNormal);
            ...
            SimpleTweenEx.moveX(cubeIgnore, 5f, 1.5f).setIgnoreTimeScale(true).setOnComplete(() => {
            ...
            int pauseCount = 0;
            SimpleTweenEx.value(gameObject, 0f, 1f, 1f).setOnUpdate((float val) => {
                pauseCount++;
            }).pause();
            ...
            SimpleTweenEx.moveLocal(cubeRound, roundCirc, 0.5f).setOnComplete(() => {
            ...
            SimpleTweenEx.moveSplineLocal(cubeSpline, roundSpline, 0.5f).setOnComplete(() => {
            ...
            var seq = SimpleTweenEx.sequence().append(SimpleTweenEx.moveX(cubeSeq, 100f, 0.2f));
            seq.append(0.1f).append(SimpleTweenEx.scaleX(cubeSeq, 2f, 0.1f));
            seq.append(() => {
            ...
            SimpleTweenEx.move(cubeBounds, new Vector3(10, 10, 10), 0.1f).setOnUpdate((float val) => {
            ...
            //SimpleTweenEx.scale(cubeLocal, new Vector3(0.5f, 0.5f, 0.5f), 0.2f).setOnComplete(() =>
            //{
            ...
            for (int i = 0; i < groupTweens.Length; i++)
            {
                ...
                groupTweens[i] = SimpleTweenEx.move(groupGOs[i], finalPos, 3f).setOnStart(() => {
                    setOnStartNum++;
                ...
                setOnComplete((object param) => {
                    ...
                    if (hasGroupTweensCheckStarted == false)
                    {
                        ...
                        SimpleTweenEx.delayedCall(gameObject, 0.1f, () => {
                ...
                if (SimpleTweenEx.description(groupTweens[i].id).trans == groupTweens[i].trans)
                    descriptionMatchCount++;
            }

            while (SimpleTweenEx.tweensRunning < groupTweens.Length)
                yield return null;

            LeanTest.expect(descriptionMatchCount == groupTweens.Length, "GROUP IDS MATCH");
            int expectedSearch = groupTweens.Length + 7;
            LeanTest.expect(SimpleTweenEx.maxSearch <= expectedSearch, "MAX SEARCH OPTIMIZED", "maxSearch:" + SimpleTweenEx.maxSearch + " should be:" + expectedSearch);
            LeanTest.expect(SimpleTweenEx.isTweening() == true, "SOMETHING IS TWEENING");

            // resume item before calling pause should continue item along it's way
            float previousXlt4 = cube4.transform.position.x;
            lt4 = SimpleTweenEx.moveX(cube4, 5.0f, 1.1f).setOnComplete(() => {
            ...
            SimpleTweenEx.rotateAround(cube3, Vector3.forward, 360f, 0.1f).setRepeat(3).setOnComplete(rotateRepeatFinished).setOnCompleteOnRepeat(true).setDestroyOnComplete(true);
            yield return new WaitForEndOfFrame();
            SimpleTweenEx.delayedCall(0.1f * 8f + 1f, rotateRepeatAllFinished);

            int countBeforeCancel = SimpleTweenEx.tweensRunning;
            SimpleTweenEx.cancel(lt1Id);
            LeanTest.expect(countBeforeCancel == SimpleTweenEx.tweensRunning, "CANCEL AFTER RESET SHOULD FAIL", "expected " + countBeforeCancel + " but got " + SimpleTweenEx.tweensRunning);
            SimpleTweenEx.cancel(cube2);

            int tweenCount = 0;
            for (int i = 0; i < groupTweens.Length; i++)
            {
                if (SimpleTweenEx.isTweening(groupGOs[i]))
                    tweenCount++;
                if (i % 3 == 0)
                    SimpleTweenEx.pause(groupGOs[i]);
                else if (i % 3 == 1)
                    groupTweens[i].pause();
                else
                    SimpleTweenEx.pause(groupTweens[i].id);
            }
            ...
            {
                if (i % 3 == 0)
                    SimpleTweenEx.resume(groupGOs[i]);
                else if (i % 3 == 1)
                    groupTweens[i].resume();
                else
                    SimpleTweenEx.resume(groupTweens[i].id);

                if (i % 2 == 0 ? SimpleTweenEx.isTweening(groupTweens[i].id) : SimpleTweenEx.isTweening(groupGOs[i]))
                    tweenCount++;
            }
            LeanTest.expect(tweenCount == groupTweens.Length, "GROUP RESUME");

            LeanTest.expect(SimpleTweenEx.isTweening(cube1) == false, "CANCEL TWEEN LTDESCR");
            LeanTest.expect(SimpleTweenEx.isTweening(cube2) == false, "CANCEL TWEEN LEANTWEEN");
            ...
            SimpleTweenEx.moveX(cube1, -5f, tweenTime).setOnUpdate((float val) => {
            ...
            SimpleTweenEx.value(gameObject, new Vector3(1f, 1f, 1f), new Vector3(10f, 10f, 10f), 1f).setOnUpdate((Vector3 val) => {
        ...
        IEnumerator lotsOfCancels()
        {
            ...
            for (int i = 0; i < aGOs.Length; i++)
            {
                ...
                cube.name = "a" + i;
                aGOs[i] = cube;
                tweensA[i] = SimpleTweenEx.move(cube, cube.transform.position + new Vector3(10f, 0, 0), 0.5f + 1f * (1.0f / (float)aGOs.Length)).id;
                SimpleTweenEx.color(cube, Color.red, 0.01f);
            }
            ...
            for (int i = 0; i < bGOs.Length; i++)
            {
                ...
                cube.name = "b" + i;
                bGOs[i] = cube;
                tweensB[i] = SimpleTweenEx.move(cube, cube.transform.position + new Vector3(10f, 0, 0), 2f).id;
            }

            for (int i = 0; i < aGOs.Length; i++)
            {
                SimpleTweenEx.cancel(aGOs[i]);
                GameObject cube = aGOs[i];
                tweensA[i] = SimpleTweenEx.move(cube, new Vector3(0, 0, i * 2f), 2f).id;
            }
            ...
            for (int i = 0; i < aGOs.Length; i++)
            {
                SimpleTweenEx.cancel(aGOs[i]);
                GameObject cube = aGOs[i];
                tweensA[i] = SimpleTweenEx.move(cube, new Vector3(0, 0, i * 2f) + new Vector3(10f, 0, 0), 2f).id;
            }

            for (int i = 0; i < bGOs.Length; i++)
            {
                SimpleTweenEx.cancel(bGOs[i]);
                GameObject cube = bGOs[i];
                tweensB[i] = SimpleTweenEx.move(cube, new Vector3(0, 0, i * 2f), 2f).id;
            }
        ...
        IEnumerator pauseTimeNow()
        {
            ...
            SimpleTweenEx.delayedCall(0.5f, () => {
                Time.timeScale = 1f;
            }).setUseEstimatedTime(true);

            SimpleTweenEx.delayedCall(1.5f, () => {
