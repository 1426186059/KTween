...
public class ST_GeneralBasics2d : MonoBehaviour {
	...
	void Start () {
		...
		SimpleTweenEx.moveX( avatarScale, avatarScale.transform.position.x + 1f, 5f).setEase(SimpleTweenType.easeOutBounce); // Simultaneously target many different tweens on the same object 

		// Move Example
		SimpleTweenEx.move( avatarMove, avatarMove.transform.position + new Vector3(1.7f, 0f, 0f), 2f).setEase(SimpleTweenType.easeInQuad);

		// Delay
		SimpleTweenEx.move( avatarMove, avatarMove.transform.position + new Vector3(2f, -1f, 0f), 2f).setDelay(3f);
	...
	void advancedExamples(){
		SimpleTweenEx.delayedCall(gameObject, 14f, ()=>{
			for(int i=0; i < 10; i++){
				...
				// Push into the wheel
				SimpleTweenEx.moveLocalZ(dude, -2f, 0.3f).setDelay(1.2f + i*0.4f).setEase(SimpleTweenType.easeSpring).setOnComplete(
					()=>{
				...
				SimpleTweenEx.moveLocalY(dude,1.17f,1.2f).setDelay(5f + i*0.2f).setLoopPingPong(1).setEase(SimpleTweenType.easeInOutQuad);
