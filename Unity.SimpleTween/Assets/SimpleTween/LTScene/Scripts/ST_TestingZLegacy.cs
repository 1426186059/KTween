...
public class ST_TestingZLegacy : MonoBehaviour {
	...
	void Awake(){
		SimpleTweenEx.init(3200); // This line is optional. Here you can specify the maximum number of tweens you will use (the default is 400).  This must be called before any use of LeanTween is made for it to be effective.
	}

	void Start () {
		ltLogo = GameObject.Find("LeanTweenLogo");
		SimpleTweenEx.delayedCall(1f, cycleThroughExamples);
...
//		alphaExample();
	...
	void cycleThroughExamples(){
		if(exampleIter==0){
			...
			if(timingType==TimingType.VariableTimeScale){
				descrTimeScaleChangeId = SimpleTweenEx.value( gameObject, 0.01f, 10.0f, 3f).setOnUpdate( (float val)=>{
					//Debug.Log("timeScale val:"+val);
					Time.timeScale = val;
				}).setEase(SimpleTweenType.easeInQuad).setUseEstimatedTime(true).setRepeat(-1).id;
			}else{
				Debug.Log("cancel variable time");
				SimpleTweenEx.cancel( descrTimeScaleChangeId );
			}
		}
		...
		SimpleTweenEx.delayedCall( gameObject, delayTime, cycleThroughExamples).setUseEstimatedTime(useEstimatedTime);
	...
	public void updateValue3Example(){
		...
		SimpleTweenEx.value( gameObject, updateValue3ExampleCallback, new Vector3(0.0f, 270.0f, 0.0f), new Vector3(30.0f, 270.0f, 180f), 0.5f ).setEase(SimpleTweenType.easeInBounce).setRepeat(2).setLoopPingPong().setOnUpdateVector3(updateValue3ExampleUpdate).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void loopTestClamp(){
		...
		SimpleTweenEx.scaleZ( cube1, 4.0f, 1.0f).setEase(SimpleTweenType.easeOutElastic).setRepeat(7).setLoopClamp().setUseEstimatedTime(useEstimatedTime);//
	}
	...
	public void loopTestPingPong(){
		...
		SimpleTweenEx.scaleY( cube2, 4.0f, 1.0f ).setEase(SimpleTweenType.easeOutQuad).setLoopPingPong(4).setUseEstimatedTime(useEstimatedTime);
		//SimpleTweenEx.scaleY( cube2, 4.0f, 1.0f, SimpleTweenEx.options().setEaseOutQuad().setRepeat(8).setLoopPingPong().setUseEstimatedTime(useEstimatedTime) );
	}

	public void colorExample(){
		GameObject lChar = GameObject.Find("LCharacter");
		SimpleTweenEx.color( lChar, new Color(1.0f,0.0f,0.0f,0.5f), 0.5f ).setEase(SimpleTweenType.easeOutBounce).setRepeat(2).setLoopPingPong().setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void moveOnACurveExample(){
		...
		SimpleTweenEx.move( ltLogo, path, 1.0f ).setEase(SimpleTweenType.easeOutQuad).setOrientToPath(true).setUseEstimatedTime(useEstimatedTime);
	}
	
	public void customTweenExample(){
		Debug.Log("customTweenExample starting pos:"+ltLogo.transform.position+" origin:"+origin);
		
		SimpleTweenEx.moveX( ltLogo, -10.0f, 0.5f ).setEase(customAnimationCurve).setUseEstimatedTime(useEstimatedTime);
		SimpleTweenEx.moveX( ltLogo, 0.0f, 0.5f ).setEase(customAnimationCurve).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
	}
	
	public void moveExample(){
		Debug.Log("moveExample");
		
		SimpleTweenEx.move( ltLogo, new Vector3(-2f,-1f,0f), 0.5f).setUseEstimatedTime(useEstimatedTime);
		SimpleTweenEx.move( ltLogo, origin, 0.5f).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void rotateExample(){
		...
		SimpleTweenEx.rotate( ltLogo, new Vector3(0f,360f,0f), 1f).setEase(SimpleTweenType.easeOutQuad).setOnComplete(rotateFinished).setOnCompleteParam(returnParam).setOnUpdate(rotateOnUpdate).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void scaleExample(){
		...
		SimpleTweenEx.scale( ltLogo, new Vector3(currentScale.x+0.2f,currentScale.y+0.2f,currentScale.z+0.2f), 1f ).setEase(SimpleTweenType.easeOutBounce).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void updateValueExample(){
		...
		pass.Add("message", "hi");
		SimpleTweenEx.value( gameObject, (Action<float, object>)updateValueExampleCallback, ltLogo.transform.eulerAngles.y, 270f, 1f ).setEase(SimpleTweenType.easeOutElastic).setOnUpdateParam(pass).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void delayedCallExample(){
		...
		SimpleTweenEx.delayedCall(0.5f, delayedCallExampleCallback).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void delayedCallExampleCallback(){
		...
		SimpleTweenEx.scale( ltLogo, new Vector3(currentScale.x-0.2f,currentScale.y-0.2f,currentScale.z-0.2f), 0.5f ).setEase(SimpleTweenType.easeInOutCirc).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void alphaExample(){
		...
		GameObject lChar = GameObject.Find ("LCharacter");
		SimpleTweenEx.alpha( lChar, 0.0f, 0.5f ).setUseEstimatedTime(useEstimatedTime);
		SimpleTweenEx.alpha( lChar, 1.0f, 0.5f ).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void moveLocalExample(){
		...
		SimpleTweenEx.moveLocal( lChar, new Vector3(0.0f,2.0f,0.0f), 0.5f ).setUseEstimatedTime(useEstimatedTime);
		SimpleTweenEx.moveLocal( lChar, origPos, 0.5f ).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
	}
	...
	public void rotateAroundExample(){
		...
		GameObject lChar = GameObject.Find ("LCharacter");
		SimpleTweenEx.rotateAround( lChar, Vector3.up, 360.0f, 1.0f ).setUseEstimatedTime(useEstimatedTime);
	}

	public void loopPause(){
		GameObject cube1 = GameObject.Find("Cube1");
		SimpleTweenEx.pause(cube1);
	}

	public void loopResume(){
		GameObject cube1 = GameObject.Find("Cube1");
		SimpleTweenEx.resume(cube1 );
	}

	public void punchTest(){
		SimpleTweenEx.moveX( ltLogo, 7.0f, 1.0f ).setEase(SimpleTweenType.punch).setUseEstimatedTime(useEstimatedTime);
	}
}
